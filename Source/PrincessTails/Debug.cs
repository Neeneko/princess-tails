...
namespace PrincessTails
{
    ...
    public class FauxVector3
    {
        public float x; public float y; public float z;

        public FauxVector3(Vector3 value)
        {
            x = value.x;
            y = value.y;
            z = value.z;
    ...
    {
        public static Dictionary<PawnRenderNode, FauxVector3> NodeMap = new Dictionary<PawnRenderNode, FauxVector3>();

        [HarmonyPostfix]
        public static Vector3 Postfix(Vector3 __result, PawnRenderNodeWorker __instance, PawnRenderNode node, PawnDrawParms parms)
        {
            if (node.debugEnabled && Patch_PawnRenderNodeWorker_ScaleFor.NodeMap.ContainsKey(node))
            {
                Vector3 vector = Vector3.one;
                vector.x *= node.Props.drawSize.x * Patch_PawnRenderNodeWorker_ScaleFor.NodeMap.TryGetValue(node).x;
                vector.z *= node.Props.drawSize.y * Patch_PawnRenderNodeWorker_ScaleFor.NodeMap.TryGetValue(node).y;
    ...
    public class Dialog_Princess_DebugRenderTree : Dialog_DebugRenderTree
    {
        public Dialog_Princess_DebugRenderTree(Pawn pawn) : base(pawn)
        {
        ...
        {
            Patch_PawnRenderNodeWorker_ScaleFor.NodeMap.Clear();
        }
    ...
    public class Dialog_DebugRenderTree : Window
    {
        ...
        public static Vector3 debugScale3 = new Vector3(1.0f, 1.0f, 1.0f);

        public override Vector2 InitialSize => new Vector2(1200f, 600f);
        ...
        private void RightRect(Rect inRect)
        {
            ...
            if (currentNode != null)
            {
                if (!(Patch_PawnRenderNodeWorker_ScaleFor.NodeMap.ContainsKey(currentNode)))
                {
                    Patch_PawnRenderNodeWorker_ScaleFor.NodeMap[currentNode] = new FauxVector3(Vector3.one);
                }
                ref float test = ref Patch_PawnRenderNodeWorker_ScaleFor.NodeMap.TryGetValue(currentNode).x;
                using (new TextBlock(TextAnchor.MiddleCenter))
                {
                ...
                rect2.y += rect2.height;
                Widgets.HorizontalSlider(rect2, ref Patch_PawnRenderNodeWorker_ScaleFor.NodeMap.TryGetValue(currentNode).x, ScaleRange, "X: " + Patch_PawnRenderNodeWorker_ScaleFor.NodeMap.TryGetValue(currentNode).x.ToStringPercent("F0"), 0.01f);
                rect2.y += rect2.height;
                Widgets.HorizontalSlider(rect2, ref Patch_PawnRenderNodeWorker_ScaleFor.NodeMap.TryGetValue(currentNode).y, ScaleRange, "Y: " + Patch_PawnRenderNodeWorker_ScaleFor.NodeMap.TryGetValue(currentNode).y.ToStringPercent("F0"), 0.01f);
                rect2.y += rect2.height;
                if ((currentNode.debugAngleOffset != 0f || currentNode.debugScale != 1f || currentNode.debugOffset != Vector3.zero || currentNode.debugLayerOffset != 0f || !currentNode.debugEnabled || currentNode.debugPivotOffset != DrawData.PivotCenter) && Widgets.ButtonText(new Rect(0f, inRect.height - 25f, inRect.width, 25f), "Reset", drawBackground: true, doMouseoverSound: true, active: true, null))
                {
                    ...
                    Patch_PawnRenderNodeWorker_ScaleFor.NodeMap[currentNode].x = Vector3.one.x;
                    Patch_PawnRenderNodeWorker_ScaleFor.NodeMap[currentNode].y = Vector3.one.y;
                    Patch_PawnRenderNodeWorker_ScaleFor.NodeMap[currentNode].z = Vector3.one.z;
                }
            }
            else
            {
